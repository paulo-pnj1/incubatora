    ...
    if t and t.get("nome"):
        with st.expander(f"📖 Teoria: {t['nome']}"):
            ...
            with c1:
                ...
                st.markdown(f'<div class="teoria-box"><h4>⚙️ Como funciona</h4><p>{t.get("como_funciona","")}</p></div>', unsafe_allow_html=True)
            with c2:
                st.markdown(f'<div class="teoria-box"><h4>✅ Quando usar</h4><p>{t.get("quando_usar","")}</p></div>', unsafe_allow_html=True)
                st.markdown(f'<div class="teoria-box"><h4>⚠️ Cuidados</h4><p>{t.get("cuidados","")}</p></div>', unsafe_allow_html=True)
...
def _treinar_regressao(df, features, target, algo, params,
    ...
    buf.seek(0)
    st.download_button("⬇️ Descarregar modelo (.pkl)", data=buf,
